...
- Release Android: запускается при создании тэга `v-*`. Собирает Android APK и публикует артефакт в GitHub Release.
- Release iOS: запускается при создании тэга `v-*`. Собирает iOS IPA и публикует артефакт в GitHub Release.

Подробности — в файлах `.github/workflows/`.

### Release Workflow

При создании тэга `v*` (например, `v1.0.0`):

1. **Android**:
   - Собирается APK.
   - Артефакт автоматически прикрепляется к релизу в GitHub.

2. **iOS**:
   - Определяется менеджер зависимостей (CocoaPods или SPM).
   - Собирается IPA.
   - Артефакт автоматически прикрепляется к релизу в GitHub.

**Примечание**: Для iOS сборка выполняется без подписи (`--no-codesign`). Для релизов в App Store требуется настройка сертификатов.
